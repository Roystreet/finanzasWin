...
$(document).on('change', '#agent', function(event) {
  ...
  $.ajax(
  {
    ...
    if (d.object == "error"){
          ...
          alertify.notify('\u2714 '+d.mensaje,d.object,2, function(){});
          $('#divembajador').hide();
          $('#nameEmb').val(d.datos.first_name);
          $('#lastnameEmb').val(d.datos.last_name);
          idofficeemb = d.datos.userid;
          emailembajador = d.datos.email;
    }
  }).fail(function(error){
    console.log(error);
    $('#load_inv').hide(30);
...
function registerticketfreshdesk(yourdomain,api_key)
{
      ...
      formdata.append('subject', $('#subject').val()+' - '+$('#asunto').val());
...
function validateData(){
  ...
  if ($("#user").val() == ""){
  ...
  }else if ($("#tipocustomer").val() == "" && $("#subject").val() == "aplicación"){
  ...
  }else if ($("#tipo").val() == "" && $("#subject").val() == "aplicación"){
  ...
  }else if ($("#so").val() == "" && $("#subject").val() == "aplicación"){
  ...
  }else if ($("#model").val() == "" && $("#subject").val() == "aplicación"){
  ...
  }else if ($("#versionso").val() == "" && $("#subject").val() == "aplicación"){
    ...
    alertify.alert('Ingrese el asunto del ticket').setHeader('<h3 style="color: orange; font-weight: bold;">Alerta</h3>');
    $(".subjectvalidate").html('El campo asunto es requerido').css("color", "#FF0000");
  }else if (desc == ""){
